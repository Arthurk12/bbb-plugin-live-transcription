...
const useEnableTranscription = (pluginApi: PluginApi, isMod: boolean) => {
  ...
  useEffect(() => {
    ...
    pluginLogger.debug('Enabling speech transcription from data channel', { logCode: 'live_transcription_enabling', extraInfo: { locale: dataChannelLocale, provider } });
    setSpeechLocale({ variables: { locale: dataChannelLocale, provider } });
  }, [shouldEnableTranscription, setSpeechLocale]);
